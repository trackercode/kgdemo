  ...
  export default {
    ...
    methods: {
      ...
      drawSales: function (){
        ...
        //---
        //循环气体统计时间 x轴,共用x轴
        var dealTime = [-1.00,0.50,1.00,1.50,2.00,2.50,3.00,3.50,4.00,4.50,5.00,5.50,6.00,6.50,7.00,7.50,8.00,8.50,9.00,9.50,10.00,10.50,11.00,11.50,12.00,12.50,13.00,13.50,14.00,14.50,15.00,15.50,16.00];
        //预测温度
        var fTempByY = [1596,1590,1587,1585,1584,1583,1582,1581,1580,1579,1579,1578,1577,1576,1575];

        //循环气体流量
        var cycArFlow = [80.463,80.501,79.758,80.133,79.866,80.012,79.962,79.772,80.056,79.689,79.430,79.656,80.079,79.857,79.614,79.679,80.079,79.435,80.098,80.358,80.246,79.875,79.776,79.939,80.541,80.413,79.807,70.331,69.240,70.621,70.094,69.904,70.027];

        //实测温度点坐标
        var tempByPoint = [{"value": "1583","xAxis": 11*2,"yAxis": "1583"},{"value": "1577","xAxis": 14*2,"yAxis": "1577"}];
        //数据转化处理，共用x轴坐标跨度对齐问处理（0.5跨度与1跨度）
        for(var i=0;i<fTempByY.length;i++){
          this.tmpArry[i]=[];
          for(var j=0;j<2;j++){
            if(j==0){
              this.tmpArry[i][j]= i*2;
            }else{
              this.tmpArry[i][j]=fTempByY[i];
            }
          }
        }
